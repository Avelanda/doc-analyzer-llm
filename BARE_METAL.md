# Run مجيب in production without Docker
...
Here you can find the scripts and known working process to run مجيب outside of a Docker container. This method of deployment is preferable for those using local LLMs and want native performance on their devices.
...
مجيب is comprised of three main sections. The `frontend`, `server`, and `collector`. When running in production you will be running `server` and `collector` on two different processes, with a build step for compilation of the frontend.
...
مجيب should now be running on `http://localhost:3001`!

## Updating مجيب

To update مجيب with future updates you can `git pull origin master` to pull in the latest code and then repeat steps 2 - 5 to deploy with all changes fully.
...
_note_ You should `pkill node` before running an update so that you are not running multiple مجيب processes on the same instance as this can cause conflicts.
